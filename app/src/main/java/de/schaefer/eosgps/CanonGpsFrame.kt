...
/**
 * Canon EOS BLE GPS data frame — the real binary format, reverse-engineered from
 * d4.C0500A.G(Location) in the Canon Camera Connect app.
 *
 ...
 *   2       4     abs(latitude) as IEEE 754 float32, big-endian
 *   6       1     E/W indicator ('E'=0x45 or 'W'=0x57)
 *   7       4     abs(longitude) as float32, big-endian
 *   11      1     altitude sign ('+'=0x2B, '-'=0x2D, or 0x00 if NaN)
 *   12      4     abs(altitude) as float32, big-endian (0.0 if NaN)
 *   16      4     unix timestamp (seconds) as int32, big-endian
 *
